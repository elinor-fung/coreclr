...
namespace BINDER_SPACE
{
    ...
    namespace
    {
        ...
        const BinderTracing::PathSource ProbingPathsToPathSource(ProbingPaths paths)
        {
            switch (paths)
            {
                case ProbingPaths::AppNiPaths:
                    return BinderTracing::PathSource::AppNativeImagePaths;
                case ProbingPaths::AppPaths:
                    return BinderTracing::PathSource::AppPaths;
                case ProbingPaths::PlatformResourceRoots:
                    return BinderTracing::PathSource::PlatformResourceRoots;
            }

            _ASSERTE(false && "Unexpected probing path source");
            return BinderTracing::PathSource::Unknown;
        }
            ...
            BindResult          *pBindResult)
        {
            ...
            for (UINT i = 0; i < pResourceRoots->GetCount(); i++)
            {
                ...
                if (BinderTracing::IsEnabled())
                    BinderTracing::PathProbed(fileName, ProbingPathsToPathSource(paths), hr);
